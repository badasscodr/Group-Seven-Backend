...
export const createVisaRecord = async (
  ...
  try {
    ...
    const docQuery = `
      SELECT id, category
      FROM documents
      WHERE id = $1 AND user_id = $2 AND category = 'visa'
    `;
    ...
    const existingVisaQuery = `
      SELECT id FROM visa_documents WHERE document_id = $1
    `;
    ...
    const insertQuery = `
      INSERT INTO visa_documents (
        user_id, document_id, visa_type, visa_number, issued_date,
        expiry_date, issuing_country, status, is_active
    ...
    return {
      id: visa.id,
      userId: visa.user_id,
      documentId: visa.document_id,
      visaType: visa.visa_type,
      visaNumber: visa.visa_number,
      issuedDate: visa.issued_date,
      expiryDate: visa.expiry_date,
      issuingCountry: visa.issuing_country,
      status: visa.status,
      notificationsSent: visa.notifications_sent || 0,
      lastNotificationDate: visa.last_notification_date,
...
export const getVisaRecords = async (
  ...
  try {
    ...
    if (userId !== undefined) {
      whereClause += ` AND vd.user_id = $${paramIndex}`;
      params.push(userId);
      paramIndex++;
    }

    if (visaType !== undefined) {
      whereClause += ` AND vd.visa_type = $${paramIndex}`;
    ...
    if (isActive !== undefined) {
      whereClause += ` AND vd.is_active = $${paramIndex}`;
      params.push(isActive);
      paramIndex++;
    }

    if (expiringWithin !== undefined) {
      whereClause += ` AND vd.expiry_date <= $${paramIndex}`;
...
export const updateVisaRecord = async (
  ...
  try {
    ...
    if (updateData.issuingCountry !== undefined) {
      setClauses.push(`issuing_country = $${paramIndex}`);
...
export const deleteVisaRecord = async (visaId: string, userId: string): Promise<boolean> => {
  try {
    const deleteQuery = 'DELETE FROM visa_documents WHERE id = $1 AND user_id = $2';
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        COUNT(CASE WHEN status = 'active' THEN 1 END) as active_visas,
        COUNT(CASE WHEN status = 'expired' THEN 1 END) as expired_visas,
        COUNT(CASE WHEN status IN ('expiring_soon', 'expiring_critical') THEN 1 END) as expiring_within_30_days,
        COUNT(CASE WHEN status = 'expiring_critical' THEN 1 END) as expiring_within_7_days,
        visa_type,
        status,
        COUNT(*) as count
      FROM visa_documents
      ${whereClause}
      GROUP BY ROLLUP(visa_type, status)
    `;
    ...
    result.rows.forEach((row) => {
      if (!row.visa_type && !row.status) {
        // Total stats row
        stats.totalVisas = parseInt(row.total_visas);
        stats.activeVisas = parseInt(row.active_visas);
        stats.expiredVisas = parseInt(row.expired_visas);
        stats.expiringWithin30Days = parseInt(row.expiring_within_30_days);
        stats.expiringWithin7Days = parseInt(row.expiring_within_7_days);
      } else if (row.visa_type && !row.status) {
        // Type breakdown
        stats.visasByType[row.visa_type as VisaType] = parseInt(row.count);
      } else if (!row.visa_type && row.status) {
        // Status breakdown
...
export const updateVisaStatuses = async (): Promise<{ updated: number; notifications: number }> => {
  ...
  try {
    ...
    const expiredQuery = `
      ...
      WHERE expiry_date < $1 AND status != 'expired' AND is_active = true
    `;
    ...
    const criticalQuery = `
      UPDATE visa_documents
      SET status = 'expiring_critical', updated_at = NOW()
      WHERE expiry_date BETWEEN $1 AND $2 AND status NOT IN ('expired', 'expiring_critical') AND is_active = true
    `;
    ...
    const soonQuery = `
      ...
      WHERE expiry_date BETWEEN $1 AND $2 AND status NOT IN ('expired', 'expiring_critical', 'expiring_soon') AND is_active = true
    `;
